              ...
              @csrf
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="link" name="link" placeholder="Masukkan Link Video" value="{{ $find_data->link }}" required>
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Materi</label>
                    <textarea class="form-control" rows="6" Placeholder="Masukkan Materi" name="teks_materi" id="teks_materi" required>{{ $find_data->teks_materi }}</textarea>
                  </div>
                  <div class="form-group">
                    ...
                    @for ($i =0; $i < $count; $i++)
                      <tr id="row{{$i}}">  
                            <td><input type="text" name="kata_kunci[]" value="{{ $arr[$i] }}" required placeholder="Masukkan kata Kunci" class="form-control name_list" /></td><td><button type="button" name="remove" id="{{ $i }}" class="btn btn-danger btn_remove">X</button></td>  
                      </tr>
                    @endfor
