              ...
              @csrf
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    @for ($i =0; $i < $count; $i++)
                      <tr id="row{{$i}}">  
                            <td><input type="text" name="kata_kunci[]" value="{{ $arr[$i] }}" placeholder="Masukkan kata Kunci" required class="form-control name_list" /></td><td><button type="button" name="remove" id="{{ $i }}" class="btn btn-danger btn_remove">X</button></td>  
                      </tr>
                    @endfor
